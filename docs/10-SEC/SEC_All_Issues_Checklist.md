...
### P1修复
1. **WiFi服务认证机制** - WiFiService添加挑战-响应认证握手
2. **EncryptedPackage元数据加密** - 发送者/接收者ID不再明文传输
3. **蓝牙特征权限** - 添加注释说明iOS蓝牙加密机制

### P2修复
1. **用户名检测超时处理** - 超时返回错误状态而非可用
2. **密钥派生context** - HKDF添加应用上下文信息
...
2. 后台Timer被系统挂起
3. 后台任务超时未保存状态
4. LanguageManager @Published非主线程修改
5. 紧急联系人缺乏双向同意
6. 互动历史缺乏知情同意
7. 信誉惩罚缺乏申诉途径
8. 黑名单无过期机制
